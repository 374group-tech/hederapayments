...
Hedera Spend Guardian is an autonomous AI agent that leverages the **Hedera Agent Kit v4** with **LangChain/LangGraph** and **DeepSeek V4 Pro** to interact with Hedera Testnet. Every tool call—whether an on-chain transfer or an off-chain API request—is intercepted by a **5-layer policy engine** before execution.
...
User Message → /api/chat
 → Policy Engine (5 Custom Policies)
   ...
   ├─ TimeWindowPolicy    → Business hours only (9:00–23:00 UTC)
...
| **TimeWindowPolicy** | Temporal guard | Restricts transactions to business hours (9:00–23:00 UTC), queries exempt |
...
| **LLM** | DeepSeek V4 Pro (via Pioneer.ai) | Cost-optimized reasoning + direct SDK for transfers |
...
// Response:
{
 ...
 "status": {
   ...
   "timeWindow": { "startHour": 9, "endHour": 23 },
...
│       ├── hcs-audit.ts            # HCS topic + message submission
